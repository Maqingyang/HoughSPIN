...
if __name__ == "__main__":
    ...
    for video_idx, det_file_path in tqdm(enumerate(det_file_paths)):
        ...
        video_folder_name = "video_%02d" %video_idx
        ...
        for frame_idx in range(len(precomputed_dets)):
            pred_betas = []
            pred_rotmat = []
            pred_camera = []
            bbox_3d = []
            ...
            if precomputed_dets[frame_idx] == []:
                continue
            for det in precomputed_dets[frame_idx]:
            ...
            img_path = os.path.join(precomputed_dets[frame_idx][0]["imgpath"]).replace('/export/guanghan/','/project/lighttrack/data/')
            ...
            renderer = Renderer(focal_length=constants.FOCAL_LENGTH, faces=smpl.faces,img=img)

            img_shape = renderer(pred_vertices, np.ones_like(img) ,mesh_trans, np.pi/6)

            frame_path = os.path.join(video_folder_name,'video_%02d_frame_%03d.png' %(video_idx,frame_idx))
            # frame_path = os.path.join('avideo_%02d_frame_%03d.png' %(video_idx,frame_idx))
            cv2.imwrite(frame_path, 255 * img_shape[:,:,::-1])
